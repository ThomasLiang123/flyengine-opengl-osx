    ...
    while (!done && !glfwWindowShouldClose(window)) {
        ...
        while (!mouse_presses.empty()) {
            if (mouse_presses.front() == 0) {
                scheme->HandleMouseDown(simd_make_float2(mouse_x, mouse_y), true);
            } else if (mouse_presses.front() == 1) {
                scheme->HandleMouseDown(simd_make_float2(mouse_x, mouse_y), false);
            }
            mouse_presses.pop();
        }
        ...
        while (!mouse_releases.empty()) {
            if (mouse_releases.front() == 0) {
                scheme->HandleMouseUp(simd_make_float2(mouse_x, mouse_y), true);
            } else if (mouse_releases.front() == 1) {
                scheme->HandleMouseUp(simd_make_float2(mouse_x, mouse_y), false);
            }
            mouse_releases.pop();
        }
