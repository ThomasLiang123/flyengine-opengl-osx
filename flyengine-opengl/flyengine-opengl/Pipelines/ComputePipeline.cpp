...
void ComputePipeline::ResetStaticBuffers() {
    ...
    vertex_buffer = std::vector<simd_float3>(num_scene_vertices);
...
void ComputePipeline::CalculateVertices() {
    ...
    for (int i = 0; i < num_scene_vertices; i++) {
        ...
        simd_float3 rel_vertex = m->GetVertex(i - m->VertexStart());
        simd_float3 abs_vertex = DragonflyUtils::TranslatePointToStandard(&mu->b, rel_vertex);
